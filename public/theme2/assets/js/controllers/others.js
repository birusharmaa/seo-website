...
(function ($) {
	...
	MPTT.document.imagesLoaded().done(function () {
		MPTT.document.trigger("images.loaded");
	});
	/**
	 * Preloader
	 */

	MPTT.document.on("images.loaded", function () {
		setTimeout(function () {
	...
	MPTT.document.on("keyup", function (e) {
		if (27 === e.keyCode) {
			$.magnificPopup.close();
		}
	});
	MPTT.document.on("click", ".popup-modal-dismiss", function (e) {
		e.preventDefault();
		$.magnificPopup.close();
	});
	...
	function half_footer_section() {
		$(".half-section-block").each(function () {
			...
			MPTT.debounceResize(function () {
				...
				if (tempPB > 0) {
					prevSection[0].style.setProperty(
						"padding-bottom",
						rectHeight + tempPB + "px",
						"important"
					);
				}

				if (tempPT > 0) {
					nextSection[0].style.setProperty(
						"padding-top",
						rectHeight + tempPT + "px",
						"important"
					);
				}

				MPTT.document.trigger("half.resize");
			});
		});
	}
	...
	MPTT.document.on("click", "[href]", function (e) {
		var $this = $(this);
