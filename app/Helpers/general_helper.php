    ...
    function get_active_menu($sidebar_menu = array())
    {
        ...
        foreach ($sidebar_menu as $key => $menu) {
            if (isset($menu["name"])) {
                ...
                if ($uri_string == "projects/all_gantt" && $menu_url == "projects/all_tasks") {
                    $found_special_active_key = $key;
                }
...
if (!function_exists('log_notification')) {
    ...
    function log_notification($event, $options = array(), $user_id = 0)
    {
        ...
        if (get_setting("log_direct_notifications")) {
            ...
            $curl_options = (get_setting("curl_options") && is_array(get_setting("curl_options"))) ? get_setting("curl_options") : array();
            ...
            try {
                $client->request('PUT', $url, $curl_options);
            } catch (\Exception $ex) {
            }
        }
    }
...
if (!function_exists("make_random_otp")) {

    function make_random_otp()
    {
...
if (!function_exists('prepare_proposal_view')) {
    ...
    function prepare_proposal_view($proposal_data)
    {
        if ($proposal_data) {
            ...
            $parser_data["COMPANY_ADDRESS"] = nl2br(get_setting("company_address"));
            $parser_data["COMPANY_PHONE"] = get_setting("company_phone");
            $parser_data["COMPANY_EMAIL"] = get_setting("company_email");
            $parser_data["COMPANY_WEBSITE"] = get_setting("company_website");
            ...
            $proposal_view = $parser->setData($parser_data)->renderString($proposal_info->content);
...
if (!function_exists('validate_header_by_cat')) {

    function validate_header_by_cat($header, $category, $subcategory)
    {
        $ci = new App_Controller();
        ...
        $header = "[" . $header . "]";
        if ($subcategory == "")
            $oldHeader = $ci->Invoices_model->getHeaderByCat($category);
        else
            $oldHeader = $ci->Invoices_model->getHeaderBySubCat($subcategory);
...
if (!function_exists('get_follorUpToday')) {

    function get_follorUpToday($uid)
    {
        $ci = new App_Controller();
        $data = $ci->Clients_model->getfollorUpToday($uid);
        return $data;
    }
}

if (!function_exists('getLeadNameById')) {

    function getLeadNameById($id)
    {
        $ci = new App_Controller();
        $data = $ci->Clients_model->getLeadNameById($id);
        return $data;
    }
}

if (!function_exists('fetchLeadFromTL')) {

    function fetchLeadFromTL($id)
    {
        $ci = new App_Controller();
        $data = $ci->Clients_model->getLeadByIdFromTL($id);
        return $data;
    }
}

if (!function_exists('getCounselorNameByIds')) {

    function getCounselorNameByIds($id)
    {
        $ci = new App_Controller();
        $datas = $ci->Clients_model->getCounselorNameByIds($id);
        return $datas;
    }
}
...
if (!function_exists('sendOtpOnMobile')) {
    ...
    function sendOtpOnMobile($fields = null)
    {
        $curl = curl_init();
        $APIKEY = 'g3Eq1odkCFSaI524ftURe67znijJyGOQvAl80mZXwuP9xTWDMHQJWkHimSRhrg4t3VLx6cNAozMeYKED';
...
if (!function_exists('cart_history')) {
    function cart_history(){
        $cart_history = new CartHistoryModel();
        $session      = session();
        $count = 0;
        if($session->has('login_user')){
            ...
            if(isset($user_data['login_type']) && $user_data['login_type'] == 'Customer'){
                ...
                $cart = $cart_history->where(['customer_id'=>$user_id, 'status'=>'Active'])->findAll();
                $count = count($cart);
            }
        }
        return $count;
...
if (!function_exists('send_mail')) {
    ...
    function send_mail($to, $subject, $message )
    {
        ...
        //check mail sending method from settings
            ...
            $email_config["SMTPPort"] = "465";
            $email_config["SMTPUser"] = "nilesh.sartia@gmail.com";
            $email_config["SMTPPass"] = "Sartia@@2022";
            $email_config["SMTPCrypto"] = "";
            // if (!$email_config["SMTPCrypto"]) {
            //     $email_config["SMTPCrypto"] = "tls"; //for old clients, we have to set this by default
            // }
        ...
        $email->setNewline("\r\n");
        $email->setCRLF("\r\n");
        $email->setFrom("nileshgautam@sartiaglobal.com", "Nilesh");
...
function email_send($to, $subject, $message, $user_name=null){   
    $email_config = array(
        'charset' => 'utf-8',
        'mailType' => 'html'
    );
    ...
    $email_config["protocol"] = "smtp";
    $email_config["SMTPHost"] = "mail.thewingshield.com";
    $email_config["SMTPPort"] = "587";
    $email_config["SMTPUser"] = "sartia@thewingshield.com";
    $email_config["SMTPPass"] = "H@i5,5(KQPkf";
    $email_config["SMTPCrypto"] = "";
    if (!$email_config["SMTPCrypto"]) {
        $email_config["SMTPCrypto"] = "tls";
    }
    $email = \CodeIgniter\Config\Services::email();
    $email->initialize($email_config);
    $email->clear(true);
    $email->setNewline("\r\n");
    $email->setCRLF("\r\n");
    $email->setFrom('sartia@thewingshield.com', 'Demo User');
    $email->setTo($to);
    $email->setSubject($subject);
    $email->setMessage($message);
    if ($email->send()) {
...
function getMenuId($value = null){
    $model = new MenuModel();
    $data = $model->select('id')->where('menu_name', $value)->first();
    return $data['id'];
    exit;
}
